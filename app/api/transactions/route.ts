...
function mapItemsFromJson(raw: string | null): CreateTransactionItemInput[] {
  ...
  try {
    ...
    return parsed
      .filter((row) => row.nama_item?.trim())
      .map((row) => ({
        item_name: row.nama_item.trim(),
        price: parseIdrAmount(row.harga),
        quantity: (typeof row.qty === 'number' && row.qty > 0) ? row.qty : 1,
      }));
  } catch {
    return [];
  }
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Filter by workspace — return ALL members' transactions
    if (workspaceIdParam) {
      query = supabaseAdmin
        .from('transactions')
        .select('*')
        .eq('workspace_id', Number(workspaceIdParam))
        .order('transaction_date', { ascending: false })
        .order('created_at', { ascending: false });
    } else {
      // Personal recap: only transactions with no workspace (personal expenses)
    ...
    if (txError) {
      console.error('List transactions error:', JSON.stringify(txError));
      return NextResponse.json<ApiError>(
        { error: 'Gagal memuat transaksi', details: txError.message } as ApiError & { details: string },
        { status: 500 }
      );
    }
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const workspaceIdRaw = formData.get('workspace_id') as string | null;
    const workspace_id = workspaceIdRaw ? Number(workspaceIdRaw) : null;
    ...
    const { data: transaction, error: insertError } = await supabaseAdmin
      ...
      .insert({
        ...
        category: workspace_id ? 'organization' : 'personal',
        merchant_name,
        total_amount,
        tax_amount,
        transaction_date,
