...
export default function SettingsPage() {
  ...
  return (
    <div style={{ height: '100%', overflowY: 'auto', background: bg, transition: 'background 0.3s' }}>
      ...
      <SectionLabel title="Pengaturan Akun" />
      <Row
      ...
      <Row
        ...
        label="Keamanan" sub="Ubah password"
        onClick={() => router.push('/settings/change-password')}
      />

      {/* App Preferences */}
      <SectionLabel title="Preferensi Aplikasi" />
      <div style={{
        display: 'flex', alignItems: 'center', gap: '14px',
        padding: '14px 16px', background: cardBg,
        borderBottom: `1px solid ${borderColor}`,
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: '11px', color: textSecondary, marginTop: '2px' }}>{darkMode ? 'Aktif' : 'Nonaktif'}</div>
        </div>
      ...
      {/* Information */}
      <SectionLabel title="Informasi" />
      <Row
        icon={<svg width="16" height="16" fill="none" stroke="#0D307F" strokeWidth="2" viewBox="0 0 24 24">
          <circle cx="12" cy="12" r="10" /><path d="M12 8v4M12 16h.01" />
        </svg>}
        label="Tentang" sub="NotaLens v1.0.0"
      />
