...
export default function WorkspaceTransactionDetailPage({
  params,
}: {
  params: Promise<{ id: string; txId: string }>
}) {
  const { id: workspaceId, txId } = use(params)
  const router = useRouter()
  const { darkMode } = useTheme()

  const [tx, setTx] = useState<TransactionWithItems & { verified?: boolean; verified_by?: number | null; verified_at?: string | null; users?: { name: string } | null } | null>(null)
  const [role, setRole] = useState<'submitter' | 'creator' | 'member' | null>(null)
  ...
  const [form, setForm] = useState({ merchant_name: '', transaction_date: '', expense_category: 'Other', tax_amount: '', total_amount: '' })
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        const res = await fetch(`/api/transaction/${txId}`, { headers: { Authorization: `Bearer ${token}` } })
        ...
        setItems((t.items ?? []).map((i: { item_name: string; quantity: number; price: number }) => ({
  ...
  const handleSave = async () => {
    ...
    if (!form.total_amount.trim()) { setSaveError('Total wajib diisi'); return }
    setSaving(true)
    const token = getToken()
    ...
    try {
      ...
      const res = await fetch(`/api/transaction/${txId}`, { method: 'PUT', headers: { Authorization: `Bearer ${token}` }, body })
      const json = await res.json()
  ...
  const handleDelete = async () => {
    ...
    const token = getToken()
    if (!token) return
    const res = await fetch(`/api/transaction/${txId}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } })
    if (res.ok) router.push(`/workspace/${workspaceId}`)
  ...
  const handleVerify = async (verified: boolean) => {
    ...
    const json = await res.json()
    if (res.ok) setTx(p => p ? { ...p, verified: json.verified, verified_at: json.verified ? new Date().toISOString() : null } : p)
    else alert(json.error || 'Gagal memverifikasi')
    setVerifying(false)
  }
  ...
  if (error || !tx) return (
    <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', height: '100%', background: bg, gap: '16px', padding: '24px' }}>
      <div style={{ fontSize: '13px', color: '#dc2626', textAlign: 'center' }}>{error || 'Transaksi tidak ditemukan'}</div>
      <button onClick={() => router.push(`/workspace/${workspaceId}`)} style={{ background: '#0D307F', color: '#fff', border: 'none', borderRadius: '12px', padding: '12px 24px', fontWeight: 700, cursor: 'pointer' }}>Kembali</button>
    </div>
  )

  const canEdit = role === 'submitter' || role === 'creator'
  const canVerify = role === 'creator'
  const isVerified = tx.verified === true
    ...
    <div style={{ height: '100%', overflowY: 'auto', background: bg, fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px', background: cardBg, borderBottom: `1px solid ${borderColor}` }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <div onClick={() => router.push(`/workspace/${workspaceId}`)} style={{ cursor: 'pointer' }}>
            <svg width="20" height="20" fill="none" stroke="#0D307F" strokeWidth="2" viewBox="0 0 24 24"><path d="M15 18l-6-6 6-6"/></svg>
          </div>
        ...
        {!editing && (
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
            {/* Verification badge */}
            {isVerified ? (
              <span style={{ fontSize: '10px', fontWeight: 700, background: '#dcfce7', color: '#16a34a', padding: '3px 10px', borderRadius: '20px', display: 'flex', alignItems: 'center', gap: '4px' }}>
              ...
              <span style={{ fontSize: '10px', fontWeight: 700, background: '#fef9c3', color: '#b45309', padding: '3px 10px', borderRadius: '20px' }}>
                Menunggu Verifikasi
              </span>
            )}
            {canEdit && (
              <button onClick={() => setEditing(true)} style={{ background: '#eff6ff', color: '#0D307F', border: 'none', borderRadius: '10px', padding: '6px 12px', fontSize: '11px', fontWeight: 700, cursor: 'pointer' }}>Edit</button>
            )}
            {canEdit && (
              <button onClick={handleDelete} style={{ background: '#fef2f2', color: '#dc2626', border: 'none', borderRadius: '10px', padding: '6px 12px', fontSize: '11px', fontWeight: 700, cursor: 'pointer' }}>Hapus</button>
            )}
          </div>
        )}
      </div>
      ...
      {saveError && (
        <div style={{ margin: '12px 16px 0', background: '#fef2f2', border: '1px solid #fca5a5', borderRadius: '10px', padding: '10px 12px', fontSize: '11px', color: '#dc2626' }}>{saveError}</div>
      )}
      ...
      <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
        ...
        {!editing && (
          <>
            {/* Verification action for creator */}
            {canVerify && (
              <div style={{ background: isVerified ? '#f0fdf4' : '#fffbeb', border: `1px solid ${isVerified ? '#bbf7d0' : '#fde68a'}`, borderRadius: '14px', padding: '14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <div style={{ fontSize: '12px', fontWeight: 700, color: isVerified ? '#16a34a' : '#b45309' }}>
                    {isVerified ? 'Sudah Diverifikasi' : 'Belum Diverifikasi'}
                  </div>
                  {isVerified && tx.verified_at && (
                    <div style={{ fontSize: '11px', color: '#64748b', marginTop: '2px' }}>
                      {formatDisplayDate(tx.verified_at)}
                    </div>
                  )}
                </div>
                <button
                  ...
                  disabled={verifying}
                  style={{
                    background: isVerified ? '#fef2f2' : '#0D307F',
                    color: isVerified ? '#dc2626' : '#fff',
                    border: 'none', borderRadius: '10px', padding: '7px 14px',
                    fontSize: '11px', fontWeight: 700, cursor: verifying ? 'not-allowed' : 'pointer',
                  }}
                >
                  {verifying ? '...' : isVerified ? 'Batalkan' : 'Verifikasi'}
                </button>
              </div>
            )}
            ...
            {/* Receipt image */}
            {receiptUrl && (
              <div onClick={() => setImgExpanded(p => !p)} style={{ borderRadius: '16px', overflow: 'hidden', cursor: 'pointer', position: 'relative' }}>
                <img src={receiptUrl} alt="Struk" style={{ width: '100%', display: 'block', borderRadius: '16px', height: imgExpanded ? 'auto' : '140px', objectFit: imgExpanded ? 'contain' : 'cover' }} />
                {!imgExpanded && (
                  <div style={{ position: 'absolute', bottom: '10px', left: '50%', transform: 'translateX(-50%)', background: 'rgba(0,0,0,0.55)', color: '#fff', fontSize: '10px', fontWeight: 700, padding: '5px 12px', borderRadius: '20px', pointerEvents: 'none' }}>
                    Tap untuk perbesar
                  </div>
                )}
              </div>
            ...
            <div style={{ background: cardBg, borderRadius: '14px', padding: '16px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
              {[
                { label: 'Nama Toko', value: tx.merchant_name || '—' },
                { label: 'Tanggal', value: formatDisplayDate(tx.transaction_date) },
                { label: 'Kategori', value: categoryLabel(tx.notes) },
                ...(tx.tax_amount != null ? [{ label: 'Tax / PPN', value: formatRp(Number(tx.tax_amount)) }] : []),
                { label: 'Total', value: formatRp(Number(tx.total_amount)) },
                { label: 'Dicatat Pada', value: formatDisplayDate(tx.created_at) },
              ].map(({ label, value }) => (
            ...
            {tx.items?.length > 0 && (
              <div style={{ background: cardBg, borderRadius: '14px', padding: '14px' }}>
                <div style={{ fontSize: '10px', fontWeight: 700, color: textSecondary, letterSpacing: '0.8px', textTransform: 'uppercase', marginBottom: '12px' }}>Item ({tx.items.length})</div>
                ...
                {tx.items.map((item, i) => (
                  <div key={item.item_id ?? i} style={{ display: 'flex', gap: '8px', alignItems: 'center', paddingBottom: '10px', borderBottom: i < tx.items.length - 1 ? `1px solid ${borderColor}` : 'none' }}>
          ...
          <>
            ...
            <div style={{ background: cardBg, borderRadius: '14px', padding: '14px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                ...
                <button onClick={() => setItems(p => [...p, { nama_item: '', qty: 1, harga: '' }])} style={{ background: '#eff6ff', color: '#0D307F', border: 'none', borderRadius: '8px', padding: '4px 10px', fontSize: '11px', fontWeight: 700, cursor: 'pointer' }}>+ Tambah</button>
              </div>
              ...
              {items.length === 0 && <p style={{ fontSize: '12px', color: textSecondary, textAlign: 'center', padding: '8px 0' }}>Belum ada item.</p>}
              {items.map((item, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '8px', paddingBottom: '8px', borderBottom: i < items.length - 1 ? `1px solid ${borderColor}` : 'none' }}>
                  <input value={item.nama_item} onChange={e => setItems(p => p.map((it, idx) => idx === i ? { ...it, nama_item: e.target.value } : it))} placeholder="Nama item"
                  ...
                  <input value={item.harga ?? ''} onChange={e => setItems(p => p.map((it, idx) => idx === i ? { ...it, harga: e.target.value } : it))} placeholder="Harga"
                  ...
                  <div onClick={() => setItems(p => p.filter((_, idx) => idx !== i))} style={{ cursor: 'pointer', color: '#dc2626', flexShrink: 0 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              <button onClick={handleSave} disabled={saving} style={{ width: '100%', background: saving ? '#6b8fd4' : '#0D307F', color: '#fff', border: 'none', borderRadius: '14px', padding: '14px', fontSize: '13px', fontWeight: 800, cursor: saving ? 'not-allowed' : 'pointer' }}>
